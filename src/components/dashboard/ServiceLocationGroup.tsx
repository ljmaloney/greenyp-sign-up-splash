...
const ServiceLocationGroup = ({ 
  ...
  const serviceCount = locationServices.length;
  ...
  return (
    <div className="bg-white border rounded-lg overflow-hidden shadow-sm">
      <Collapsible open={isOpen} onOpenChange={() => (hasMultipleServices || !hasServices) && onToggle()}>
        <div className="bg-greenyp-50 p-4 border-b border-greenyp-100">
          <div className="flex items-center justify-between">
            <CollapsibleTrigger asChild>
              <div className={`flex items-center flex-1 ${(hasMultipleServices || !hasServices) ? 'cursor-pointer' : 'cursor-default'}`}>
                <MapPin className="w-4 h-4 mr-2 text-greenyp-600" />
                <h3 className="font-semibold text-gray-900">{locationName}</h3>
                <span className="ml-2 text-sm text-gray-500">
                  ({serviceCount} service{serviceCount !== 1 ? 's' : ''})
                </span>
                {(hasMultipleServices || !hasServices) && (
                  <div className="ml-2">
          ...
          <div className="divide-y bg-white">
            {!hasServices ? (
              <div className="p-6 text-center">
                <p className="text-gray-500 mb-4">No Services Configured</p>
                <Button 
                  onClick={() => onAddService(locationId)}
